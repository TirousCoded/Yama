...
TEST(CallSigTests, Equality_DiffParamCounts) {
...
// NOTE: these fmt tests exist as yama::callsig's fmt behaviour
...
// TODO: later on, when we replace links w/ consts fully, add tests for when a callsig
//       illegally refers to constants which are not valid object types
//
//       also gonna need static verif tests for that too!

TEST(CallSigTests, Fmt_OutOfBoundsIndices) {
    const auto consts =
        yama::const_table()
        .add_primitive_type("a"_str)
        .add_primitive_type("b"_str)
