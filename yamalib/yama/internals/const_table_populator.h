...
namespace yama::internal {
    ...
    public:
        ...
        callsig build_callsig_for_fn_type(const ctype& t);


    private:
        template<const_type C>
        inline std::optional<const_t> _find_existing_c(const const_table& consts, const const_data_of_t<C>& x) const noexcept;
        template<const_type C>
        inline std::optional<const_t> _find_existing_type_c(const const_table& consts, const str& qualified_name) const noexcept;
    };

    template<const_type C>
    inline std::optional<const_t> const_table_populator::_find_existing_c(const const_table& consts, const const_data_of_t<C>& x) const noexcept {
    ...
    template<const_type C>
    inline std::optional<const_t> const_table_populator::_find_existing_type_c(const const_table& consts, const str& qualified_name) const noexcept {
        for (const_t i = 0; i < consts.consts.size(); i++) {
            if (const auto c = consts.get<C>(i)) {
