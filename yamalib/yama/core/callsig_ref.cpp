...
yama::callsig_ref::callsig_ref(const callsig& info, const_table_ref consts) 
    : _info(info), 
    _consts(consts) {}

const yama::callsig& yama::callsig_ref::info() const noexcept {
    return *_info;
}

size_t yama::callsig_ref::params() const noexcept {
...
std::string yama::callsig_ref::fmt() const {
    std::string params_s{};
    bool first = true;
    for (size_t i = 0; i < params(); i++) {
        if (!first) {
