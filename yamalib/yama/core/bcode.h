...
namespace yama::bc {
    ...
    enum class opcode : uint8_t {
        ...
        // Synopsis: put_type_const R(A) Kt(B)
        //
        ...
        // Synopsis: default_init R(A) Kt(B)
        //
        ...
        // Synopsis: conv R(A) R(B) Kt(C)
        //
        // conv performs conv(R(A), R(B), Kt(C)).
        //
        // Panic Conditions:
        //      - If conversion fails.
        //
        ...
        //      - R(A) must be in-bounds.
        //      - R(B) must be in-bounds (unless newtop.)
        //      - Kt(C) must be in-bounds.
        //      - Kt(C) must be a type constant.
        //      - R(B) and Kt(C) must agree on type (unless reinit) (unless newtop.)
        //      - Pushing must not overflow (unless not newtop.)
        conv,
...
namespace yama::bc {
    ...
    // A single Yama bcode instruction.
    struct instr final {
        opcode opc;
        uint8_t A;
        union {
...
namespace yama::bc {
    ...
    public:
        ...
        code& add_conv(uint8_t A, uint8_t B, uint8_t C, bool reinit = false);
    ...
    public:
        ...
        code_writer& add_conv(uint8_t A, uint8_t B, uint8_t C, bool reinit = false);
        code_writer& add_call(uint8_t A, uint8_t B, bool reinit = false);
        code_writer& add_call_nr(uint8_t A);
        code_writer& add_ret(uint8_t A);
        // label_id may specify labels added later.
        code_writer& add_jump(label_id_t label_id);
        // label_id may specify labels added later.
        code_writer& add_jump_true(uint8_t A, label_id_t label_id);
        // label_id may specify labels added later.
        code_writer& add_jump_false(uint8_t A, label_id_t label_id);
        ...
        // Index field is replaced during sym output.
        void autosym(const sym& x);
        // Fails quietly if no autosym is bound.
        void drop_autosym() noexcept;
