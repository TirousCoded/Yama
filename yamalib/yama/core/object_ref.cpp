...
std::string yama::object_ref::fmt() const {
    ...
    if (t.kind() == kind::primitive) {
        ...
        else if (x == ptype::uint) {
            result = std::format("{} ({})", t.fullname(), as_uint());
        }
        else if (x == ptype::float0) {
            result = std::format("{} ({})", t.fullname(), as_float());
        }
        else if (x == ptype::bool0) {
            result = std::format("{} ({})", t.fullname(), as_bool());
        }
        else if (x == ptype::char0) {
            result = std::format("{} ({})", t.fullname(), taul::fmt_unicode(as_char()));
        }
        else YAMA_DEADEND;
    }
