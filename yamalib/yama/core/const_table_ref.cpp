...
bool yama::const_table_ref::is_stub(const_t x) const noexcept {
    return
        x < size()
...
std::optional<yama::const_type> yama::const_table_ref::const_type(const_t x) const noexcept {
...
std::string yama::const_table_ref::fmt_const(const_t x) const {
    std::string a{};
    static_assert(const_types == 9);
...
std::string yama::const_table_ref::fmt(const char* tab) const {
    YAMA_ASSERT(tab);
    std::string result{};
    result += std::format("const_table_ref ({} consts)", size());
