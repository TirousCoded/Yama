...
void yama::dm::static_verifier::_dump_cfg(const bc::code& bcode) {
    YAMA_LOG(dbg(), static_verif_c, "dumping CFG:");
    for (size_t i = 0; i < bcode.count(); i++) {
        ...
        YAMA_LOG(dbg(), static_verif_c, "{}", b.fmt());
        constexpr const char* tab = "    ";
        YAMA_LOG(dbg(), static_verif_c, "{}processed            : {}", tab, b.processed);
        YAMA_LOG(dbg(), static_verif_c, "{}processed-by         : {}", tab,
            [&]() -> std::string {
                if (!b.processed) {
                    return "n/a";
                }
        ...
        YAMA_LOG(dbg(), static_verif_c, "{}initial register states:", tab);
        if (!b.processed) {
            YAMA_LOG(dbg(), static_verif_c, "{}{}n/a", tab, tab);
        }
            ...
            for (size_t j = 0; j < b.initial_reg_set.size(); j++) {
                YAMA_LOG(dbg(), static_verif_c, "{}{}register {}: {}", tab, tab, j, b.initial_reg_set[j]);
            }
        }
        YAMA_LOG(dbg(), static_verif_c, "{}final register states:", tab);
        if (!b.processed) {
            YAMA_LOG(dbg(), static_verif_c, "{}{}n/a", tab, tab);
        }
            ...
            for (size_t j = 0; j < b.final_reg_set.size(); j++) {
                YAMA_LOG(dbg(), static_verif_c, "{}{}register {}: {}", tab, tab, j, b.final_reg_set[j]);
            }
        }
    }
}
...
bool yama::dm::static_verifier::_verify_arg_registers_in_bounds(const type_info& subject, const bc::code& bcode, size_t i) {
    ...
    if (!internal::range_overlap<size_t>(0, bcode.count(), args_index, size_t(args_index) + size_t(args_count))) {
        YAMA_LOG(
            dbg(), static_verif_c,
...
bool yama::dm::static_verifier::_verify_param_arg_registers_are_correct_number_and_types(const type_info& subject, const bc::code& bcode, _cfg_block& block, size_t i) {
    ...
    YAMA_ASSERT(internal::range_overlap<size_t>(0, bcode.count(), args_index, args_index + args_count));
